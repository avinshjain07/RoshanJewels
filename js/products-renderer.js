...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderSearchAndFilters() {
    ...
    if (pageContext.mode === "collection") {
      // For collection pages (e.g. Gold), show product types as filters
  ...
  function renderProducts() {
    ...
    grid.innerHTML = filtered.map(p => {
      ...
      const whatsappText = `Hello Roshan Jewel, I would like to inquire about the ${p.name} from the ${p.collection} (${p.subcategory.trim()}). Could you please share more details?`;
      ...
      return `
        <div class="product-card" id="product-${p.id}">
          ...
          <div class="product-img">
            <img src="${imagePath}" alt="${p.name} - ${p.collection}" loading="lazy">
          </div>
          <div class="product-info">
            ...
            <a href="${whatsappUrl}" class="btn-small" target="_blank">Inquire Now</a>
          </div>
        </div>
      `;
    }).join("");

    // 4. Trigger animations (from main.js)
